...
) -> tuple[float, float, list[Rectangle], Pose, int, float]:
    settings = _settings(cfg, "perpendicular", tight)
    wall = float(cfg["world"]["wall_thickness"])
    spot_width = float(settings["spot_width"])
...
def _parallel_layout(
    ...
    return width, height, obstacles, goal, aisle_y
...
def generate_parking_scenario(
    ...
    if name not in PARKING_TYPES:
        raise ValueError(f"unknown parking type {name!r}; expected one of {PARKING_TYPES}")
    cfg = config if config is not None else load_config("parking")
    ...
    for _ in range(attempts):
        layout = (
            BASE_LAYOUTS[int(rng.integers(0, len(BASE_LAYOUTS)))] if tight else name
        )
        if layout.startswith("perpendicular"):
            reverse = layout.endswith("reverse")
            width, height, obstacles, goal, _, aisle_y = _perpendicular_layout(
                rng, cfg, car, reverse, tight
            )
        else:
            reverse = True  # a parallel bay can only be entered by reversing in
            width, height, obstacles, goal, aisle_y = _parallel_layout(rng, cfg, car, tight)
        ...
        scenario = ParkingScenario(
            ...
            config=cfg, hybrid_config=hybrid_cfg,
        )
