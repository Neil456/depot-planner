...
- [x] **Step 7 (optional): C++ core for grid A*** — done. `cpp/grid_astar.cpp` via
      ...
      expansion counts; 13-30x faster. *(Superseded by the C++ port below: that single
      translation unit became the `cpp/` library, and the build no longer skips silently.)*
...
`python3 scripts/check_equivalence.py` compares the C++ core against the Python reference
on every battery seed of both tiers. The whole sweep takes about four minutes:

| section | comparisons | result |
| --- | --- | --- |
| `grid` | 600 | identical paths, costs and expansion counts |
| `fields` | 20 | distance fields bit-identical to scipy, Dijkstra fields identical |
| `spacetime` | 1260 | identical plans, both planners, both tiers, three start steps |
| `hybrid` | 180 | identical plans; every C++ plan also passes the independent checker |
| `runner` | 420 | identical episodes, every metric |

Not one equal-cost tie-break difference in any section.

## Definition of done ([`TASK_CPP.md`](TASK_CPP.md))

- [x] **All planners run in C++ by default; the Python reference is retained and tested
      against.** `depot_planner/core.py` resolves every call to the core whenever it is
      built, which `pip install -e .` always does. `--backend python` runs the reference
      end to end, and `scripts/check_equivalence.py` compares the two on every battery seed
      of both tiers.
- [x] **Batteries regenerated with the C++ backend; REPORT.md updated.** All four tiers were
      rerun on both backends; the hard tier is reported side by side because it is the one
      place where the implementation changes the result.
- [x] **GoogleTest + pytest + sanitizer CI all green.** 68 GoogleTest cases, 236 pytest
      tests, and the same GoogleTest suite under AddressSanitizer and
      UndefinedBehaviorSanitizer. CI runs all three as separate jobs, plus
      `clang-format --Werror`.
- [x] **Benchmark table in the README.** Generated by `scripts/bench_cpp.py`, medians of
      five runs per backend with setup excluded, and an `identical plans` column the
      benchmark measures rather than asserts.
- [x] **PROGRESS.md and DECISIONS.md accurate.** One decisions section per step, including
      the divergences that were found and what was done about them.
...
- The C++ core's first version matched every cost but differed on 34 of 600 *paths*,
  ...
  a test — and that test is the shape every later port was held to.
- **`math.hypot` is not the platform `hypot`.** CPython has its own correctly-rounded norm;
  `std::hypot` disagreed with it on 0.6% of 200k random pairs by one ulp, which is enough to
  reorder hybrid A*'s open list. Python's float `%` and `//` are not C's either. All three
  are reimplemented in `cpp/include/depot/numeric.hpp`; numpy's array `cos` and `sin` were
  measured against the scalar ones and do agree here.
